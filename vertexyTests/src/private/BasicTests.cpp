...
int TestSolvers::solveShortestPath(int times, int seed, tuple<int, int> limits, int numVertices, bool printVerbose)
{
	int nErrorCount = 0;
	...
	EATEST_VERIFY(iteration == 10);

	return nErrorCount;
}

int TestSolvers::solveShortestPath_Max(int times, int seed, bool printVerbose)
{
	int nErrorCount = 0;
	...
	const vector<int> edge_Open = { 1 };
	const vector<int> edge_Closed = { 0 };
	tuple<int, int> distLimits = make_tuple(0, 3);
	solver.makeConstraint<ShortestPathConstraint>(typeData, origin, target, openData, edge_Closed, distLimits);
	
	for (int iteration = 0; iteration < times; ++iteration)
	{
		EATEST_VERIFY(solver.solve() == EConstraintSolverResult::Solved);
		...
		int pathLength = TopologySearchAlgorithm::shortestPathTo(topology, sourceVertex, targetVertex, path);
		EATEST_VERIFY(pathLength != INT_MAX);
		EATEST_VERIFY(pathLength > 0 && pathLength <= 3);
...
int TestSolvers::solveShortestPath_Min(int times, int seed, bool printVerbose)
{
	int nErrorCount = 0;
	constexpr int numVertices = 6;
	...
	int solutionKey = 0;
	hash_map<int, int> solution_Positions;
	while (solver.solve() != Vertexy::EConstraintSolverResult::Unsatisfiable)
	{
		EATEST_VERIFY(solver.getCurrentStatus() == EConstraintSolverResult::Solved);
...
int TestSolvers::solveShortestPath_Range(int times, int seed, bool printVerbose)
{
	int nErrorCount = 0;
	constexpr int numVertices = 6;
	...
	// Need this to actually do interesting things!
	ConstraintSolver solver(TEXT("solveShortestPath_Min"), seed);

	SolverVariableDomain typeDomain(0, 2);
	auto typeData = solver.makeVariableGraph(TEXT("TypeData"), ITopology::adapt(topology), typeDomain, TEXT("type"));

	const vector<int> origin{ 0 };
	const vector<int> target{ 2 };
	solver.setInitialValues(typeData->get(0), origin);
	solver.setInitialValues(typeData->get(1), target);

	hash_map<int, tuple<int, int>> globalCardinalities;
	globalCardinalities[0] = make_tuple(1, 1);
	globalCardinalities[2] = make_tuple(1, 1);
	solver.cardinality(typeData->getData(), globalCardinalities);
	...
	tuple<int, int> distLimits = make_tuple(3, 4);
	solver.makeConstraint<ShortestPathConstraint>(typeData, origin, target, openData, closedPath, distLimits);

	solver.solve();
	EATEST_VERIFY(solver.getCurrentStatus() == EConstraintSolverResult::Solved);

	vector<int> path;
	const int pathLength = TopologySearchAlgorithm::shortestPathTo(topology, 0, 1, path);
	EATEST_VERIFY(pathLength != INT_MAX);
	EATEST_VERIFY(pathLength >= 3);
	EATEST_VERIFY(pathLength <= 4);
