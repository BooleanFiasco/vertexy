...
PathDistanceConstraint* PathDistanceConstraint::PathDistanceFactory::construct(
	...
	const tuple<int, int>& distanceLimits)
{
	...
	return new PathDistanceConstraint(params, vertexData, sourceMask, needReachableMask, edgeData, edgeBlockedMask, distanceLimits);
}

PathDistanceConstraint::PathDistanceConstraint(
	const ConstraintFactoryParams& params,
...
bool PathDistanceConstraint::getGraphRelations(const vector<Literal>& literals, ConstraintGraphRelationInfo& outRelations) const
{
...
bool PathDistanceConstraint::initialize(IVariableDatabase* db)
{
...
void PathDistanceConstraint::reset(IVariableDatabase* db)
{
...
bool PathDistanceConstraint::onVariableNarrowed(IVariableDatabase* db, VarID variable, const ValueSet& prevValue, bool& removeWatch)
{
...
bool PathDistanceConstraint::EdgeWatcher::onVariableNarrowed(IVariableDatabase* db, VarID var, const ValueSet& prevValue, bool& removeHandle)
{
...
bool PathDistanceConstraint::propagate(IVariableDatabase* db)
{
...
bool PathDistanceConstraint::processVertexVariableChange(IVariableDatabase* db, VarID variable)
{
...
void PathDistanceConstraint::addSource(VarID source)
{
	int vertex = m_variableToSourceVertexIndex[source];

#if REACHABILITY_USE_RAMAL_REPS
	constexpr bool reportReachability = true;	// Same as ReachabilityConstraint defaults
...
bool PathDistanceConstraint::removeSource(IVariableDatabase* db, VarID source)
{
	...
	auto checkReachability = [&](int vertex, int parent)
		{
			if (sourceData.maxReachability->isReachableWithinLimits(vertex, m_distanceLimits))
			{
...
void PathDistanceConstraint::recordVertexWithinLimits(const IVariableDatabase* db, int destination, int source)
{
...
void PathDistanceConstraint::eraseVertexWithinLimits(const IVariableDatabase* db, int destination)
{
	if (m_withinLimitsTimestampsToCommit.find(destination) != m_withinLimitsTimestampsToCommit.end())
	{
		vxy_assert(false);
...
void PathDistanceConstraint::commitVertexWithinLimitsTimestamps(const IVariableDatabase* db)
{
...
void PathDistanceConstraint::revertVertexWithinLimitsTimestamps()
{
	m_withinLimitsTimestampsToCommit.clear();
}

void PathDistanceConstraint::updateGraphsForEdgeChange(IVariableDatabase* db, VarID variable)
{
...
void PathDistanceConstraint::onReachabilityChanged(int vertexIndex, VarID sourceVar, bool inMinGraph)
{
...
void PathDistanceConstraint::onDistanceChanged(int vertexIndex, VarID sourceVar, int distanceFromSource, bool inMinGraph)
{
	vxy_assert(!m_backtracking);
	vxy_assert(!m_explainingSourceRequirement);
...
void PathDistanceConstraint::backtrack(const IVariableDatabase* db, SolverDecisionLevel level)
{
	vxy_assert(!m_edgeChangeFailure);
	m_edgeProcessList.clear();
	m_vertexProcessList.clear();
...
PathDistanceConstraint::EReachabilityDetermination PathDistanceConstraint::determineReachability(const IVariableDatabase* db, int vertexIndex)
{
	VarID vertexVar = m_sourceGraphData->get(vertexIndex);
	for (auto it = m_reachabilitySources.begin(), itEnd = m_reachabilitySources.end(); it != itEnd; ++it)
	{
		...
		if (it->second.minReachability->isReachableWithinLimits(vertexIndex, m_distanceLimits, TEXT("min")))
		{
			// Keep track of timestamps on which this vertex is within distance limits
			int sourceVertex = it->second.minReachability->getSourceVertex();
			recordVertexWithinLimits(db, vertexIndex, sourceVertex);
		...
		else if (it->second.maxReachability->isReachableWithinLimits(vertexIndex, m_distanceLimits, TEXT("max")))
		{
			// Keep track of timestamps on which this vertex is within distance limits
			int sourceVertex = it->second.minReachability->getSourceVertex();
			recordVertexWithinLimits(db, vertexIndex, sourceVertex);
	...
	eraseVertexWithinLimits(db, vertexIndex);
...
void PathDistanceConstraint::onExplanationGraphEdgeChange(bool edgeWasAdded, int from, int to)
{
...
void PathDistanceConstraint::explainNoReachability(const NarrowingExplanationParams& params, vector<Literal>& outExplanation) const
{
	//outExplanation = IVariableDatabase::defaultExplainer(params);
	//return;
...
void PathDistanceConstraint::explainNotWithinLimits(const NarrowingExplanationParams& params, vector<Literal>& outExplanation) const
{
	//outExplanation = IVariableDatabase::defaultExplainer(params);
	//return;
	...
	// First things first, make sure we ever had a valid state for this vertex
	/*
	auto conflictVertexLastTimestampIt = m_lastWithinLimitsTimestamp.find(conflictVertex);
	if (conflictVertexLastTimestampIt == m_lastWithinLimitsTimestamp.end())
	{
		// We never had a valid value so all we can do is fully reset and try again
		for (int potentialSrcIndex = 0; potentialSrcIndex < m_initialPotentialSources.size(); ++potentialSrcIndex)
		{
			VarID potentialSource = m_initialPotentialSources[potentialSrcIndex];
			int sourceVertex = m_variableToSourceVertexIndex.find(potentialSource)->second;
			...
			if (!db->getPotentialValues(potentialSource).anyPossible(m_sourceMask))
			{
				// best explanation is that any source that isn't a source currently could make it reachable
				outExplanation.push_back(Literal(potentialSource, m_sourceMask));
			}
		}
		return;
	...
	*/
	...
	for (int potentialSrcIndex = 0; potentialSrcIndex < m_initialPotentialSources.size(); ++potentialSrcIndex)
	{
		...
		if (db->getPotentialValues(potentialSource).anyPossible(m_sourceMask))
		{
			// First things first, look to see if we have a valid timestamp for conflictVertex => sourceVertex
			SolverTimestamp rewindTimestamp = -1;
			auto conflictVertexLastTimestampIt = m_lastWithinLimitsTimestamp.find(conflictVertex);
			if (conflictVertexLastTimestampIt != m_lastWithinLimitsTimestamp.end())
			{
				auto sourceVertexLastTimestampIt = conflictVertexLastTimestampIt->second.find(sourceVertex);
				if (sourceVertexLastTimestampIt != conflictVertexLastTimestampIt->second.end())
				{
					// Found a valid timestamp for this source/destination pair!
					rewindTimestamp = sourceVertexLastTimestampIt->second;
				}
			}

			// If we didn't find a timestamp then we never had a good value for this pair, best explanation is
			// that we really just don't want this thing to be a source.
			...
			m_explanationGraph->rewindUntil(rewindTimestamp);

			vector<int> validPath;
			m_shortestPathAlgo.find<BacktrackingDigraphTopology>(*m_explanationGraph, sourceVertex, conflictVertex, validPath);

			m_explanationGraph->fastForward();
			vxy_assert(get<0>(m_distanceLimits) <= (validPath.size() - 1) && get<1>(m_distanceLimits) >= (validPath.size() - 1));
			
			int openedEdges = 0;
			for (int i = 0; i < (validPath.size() - 1); ++i)
			{
				// Compare edges to see which ones are disabled now and add them to the explanation
				int edge = m_edgeGraph->getVertexForSourceEdge(validPath[i], validPath[i + 1]);
				const auto& edgeVar = m_edgeGraphData->get(edge);
				if (edgeVarsRecorded.find(edgeVar) == edgeVarsRecorded.end())
				{
					if (!db->getValueBefore(edgeVar, params.timestamp).anyPossible(m_edgeOpenMask))
					{
						edgeVarsRecorded.insert(edgeVar);
						outExplanation.push_back(Literal(edgeVar, m_edgeOpenMask));
						++openedEdges;
...
void PathDistanceConstraint::explainRequiredSource(const NarrowingExplanationParams& params, VarID removedSource, vector<Literal>& outExplanation)
{
	//outExplanation = IVariableDatabase::defaultExplainer(params);
	//return;
	...
	auto searchCallback = [&](int vertex, int parent, int edgeIndex)
		{
			if (ourReachability->isReachableWithinLimits(vertex, m_distanceLimits))
			{
				VarID vertexVar = m_sourceGraphData->get(vertex);
				if (!db->anyPossible(vertexVar, m_notReachableMask))
				{
					...
					for (auto it = m_reachabilitySources.begin(), itEnd = m_reachabilitySources.end(); it != itEnd; ++it)
					{
						if (it->first != sourceVar && it->first != vertexVar && it->second.maxReachability->isReachableWithinLimits(vertex, m_distanceLimits))
						{
					...
					{
						vxy_assert(m_reachabilitySources[sourceVar].maxReachability->isReachableWithinLimits(vertex, m_distanceLimits));
...
void PathDistanceConstraint::sanityCheckUnreachable(IVariableDatabase* db, int vertexIndex)
{
#if SANITY_CHECKS
	// For each source that could possibly exist...
	for (VarID potentialSource : m_initialPotentialSources)
...
void PathDistanceConstraint::sanityCheckNotWithinLimits(IVariableDatabase* db, int vertexIndex)
{
#if SANITY_CHECKS
	// For each source that could possibly exist...
	for (VarID potentialSource : m_initialPotentialSources)
...
vector<VarID> PathDistanceConstraint::getConstrainingVariables() const
{
	vector<VarID> out;
	for (int i = 0; i < m_sourceGraph->getNumVertices(); ++i)
	{
...
bool PathDistanceConstraint::checkConflicting(IVariableDatabase* db) const
{
	// TODO
	return false;
}
