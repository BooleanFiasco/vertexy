...
namespace Vertexy
{
	 ...
	 */
	class PathDistanceConstraint : public IBacktrackingSolverConstraint
	{
	public:
		PathDistanceConstraint(const ConstraintFactoryParams& params,
		...
		struct PathDistanceFactory
		{
			static PathDistanceConstraint* construct(
		...
		using Factory = PathDistanceFactory;

		virtual EConstraintType getConstraintType() const override { return EConstraintType::PathDistance; }
	...
	protected:
		...
		void explainNotWithinLimits(const NarrowingExplanationParams& params, vector<Literal>& outExplanation) const;
		...
		void onDistanceChanged(int vertexIndex, VarID sourceVar, int distanceFromSource, bool inMinGraph);
		void sanityCheckNotWithinLimits(IVariableDatabase* db, int vertexIndex);
		...
		{
		public:
			EdgeWatcher(PathDistanceConstraint& parent)
				: m_parent(parent)
			{
			}
		...
		protected:
			PathDistanceConstraint& m_parent;
		};
		...
		mutable ShortestPathAlgorithm m_shortestPathAlgo;
